"""Parakeet TDT V3 baseline evaluation.

Uses parakeet-mlx (native MLX implementation of NVIDIA's Parakeet TDT 0.6B v3).
FastConformer encoder + Token-and-Duration Transducer decoder, bfloat16.

Usage:
    python -m moonshine.baselines.parakeet_baseline                    # quick test (10 samples)
    python -m moonshine.baselines.parakeet_baseline --full             # full LibriSpeech test-clean
    python -m moonshine.baselines.parakeet_baseline --max-samples 250  # custom sample count
"""
...
def make_transcribe_fn(model_path=MODEL):
    ...
    print(f"Loading Parakeet TDT V3 from {model_path}...")
...
def main():
    parser = argparse.ArgumentParser(description="Parakeet TDT V3 evaluation")
    parser.add_argument(
        "--dataset",
    ...
    if len(results) > 1:
        print(f"\n{'=' * 70}")
        print(f"  Summary: Parakeet TDT V3 (MLX)")
        print(f"{'=' * 70}")
