"""Parakeet TDT V2 baseline evaluation (6-bit quantized).

Loads a LOCAL 6-bit MLX checkpoint of NVIDIA's Parakeet TDT 0.6B v2, built by
moonshine/recipes/parakeet/quantize_v2_8bit.py --bits 6 (nn.quantize group_size=64,
bits=6). Only nn.Linear/nn.Embedding are quantized; Conv1d/BatchNorm/LSTM stay bf16.

Cold load in this process, so the eval's RAM number reflects the quantized model.

Usage:
    python -m moonshine.baselines.parakeet_v2_6bit_baseline                    # quick test (10 samples)
    python -m moonshine.baselines.parakeet_v2_6bit_baseline --full             # full LibriSpeech test-clean
    python -m moonshine.baselines.parakeet_v2_6bit_baseline --max-samples 250  # custom sample count
"""
...
MODEL_DIR = "moonshine/models/parakeet-tdt-0.6b-v2-6bit"
GROUP_SIZE = 64
BITS = 6
...
def make_transcribe_fn(model_dir=MODEL_DIR):
    ...
    print(f"Loading Parakeet TDT V2 6-bit from {model_dir}...")
    ...
    print(f"Loaded: {total/1e6:.1f}M params (6-bit quantized linears + bf16, MLX)")
...
def main():
    parser = argparse.ArgumentParser(description="Parakeet TDT V2 (6-bit) evaluation")
    parser.add_argument(
        "--dataset",
    ...
    for ds in datasets:
        result = evaluate(
            ...
            dataset_name=ds,
            model_name="parakeet-tdt-v2-6bit",
    ...
    if len(results) > 1:
        print(f"\n{'=' * 70}")
        print(f"  Summary: Parakeet TDT V2 6-bit (MLX)")
        print(f"{'=' * 70}")
