"""Parakeet TDT V2 baseline evaluation (bf16).

Uses parakeet-mlx (native MLX implementation of NVIDIA's Parakeet TDT 0.6B v2).
FastConformer encoder + Token-and-Duration Transducer decoder, bfloat16.

The v2 checkpoint is fp32 on disk (~2.47 GB), but parakeet-mlx's from_pretrained
casts to bfloat16 by default; that is the config measured here.

Usage:
    python -m moonshine.baselines.parakeet_v2_baseline                    # quick test (10 samples)
    python -m moonshine.baselines.parakeet_v2_baseline --full             # full LibriSpeech test-clean
    python -m moonshine.baselines.parakeet_v2_baseline --max-samples 250  # custom sample count
"""
...
def make_transcribe_fn(model_path=MODEL):
    ...
    print(f"Loading Parakeet TDT V2 from {model_path}...")
    ...
    print(f"Loaded: {total/1e6:.1f}M params (bfloat16, MLX)")
...
def main():
    parser = argparse.ArgumentParser(description="Parakeet TDT V2 (bf16) evaluation")
    parser.add_argument(
        "--dataset",
    ...
    for ds in datasets:
        result = evaluate(
            ...
            dataset_name=ds,
            model_name="parakeet-tdt-v2-bf16",
    ...
    if len(results) > 1:
        print(f"\n{'=' * 70}")
        print(f"  Summary: Parakeet TDT V2 bf16 (MLX)")
        print(f"{'=' * 70}")
