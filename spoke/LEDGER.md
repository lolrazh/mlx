...
> Last updated: 2026-07-07 (**NEW ACCURACY CHAMPION: Gemma 4 E4B-it, v5-split 2000 steps → 96 core / 79 broad / 86 v5, 0 fails on all 3 sets (finding #106).** Beats Qwen3-4B champion on the two weighty sets: broad58 +5.2 (79 vs 74, AND 0 vs 2 hard fails), v5-131 +4.6 (86 vs 82); loses core23 by ~1 example (96 vs 100). COSTS: 3-4x slower (0.9-1.1s vs 0.25s), deployment needs unvalidated gemma4→MLX text-decoder conversion, single-seed (confirm w/ 2-3 seeds). Prior "Champion: Qwen3-4B T4-v5split 100/74/82" now the SPEED/deploy champion, Gemma the ACCURACY champion. Gemma4 fine-tune required a full-model-load + torch2.8 + regex-target fix (finding #106). Modal workspace: sandy-36852; flaky local network cancels attached modal runs + two overlapping `modal run` clients collide (APP_STATE_STOPPED) — one clean client, no head-pipe, retry loops mandatory.)
