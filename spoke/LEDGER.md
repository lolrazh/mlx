...
> Last updated: 2026-07-07 (**NEW ACCURACY CHAMPION: Gemma 4 E4B-it, v5-split 2000 steps → 96 core / 79 broad / 86 v5, 0 fails on all 3 sets (finding #106).** Beats Qwen3-4B champion on the two weighty sets: broad58 +5.2 (79 vs 74, AND 0 vs 2 hard fails), v5-131 +4.6 (86 vs 82); loses core23 by ~1 example (96 vs 100). COSTS: 3-4x slower (0.9-1.1s vs 0.25s), deployment needs unvalidated gemma4→MLX text-decoder conversion, single-seed. **UPDATE (finding #107): 3-seed confirm PASSES — margin over Qwen is real (3-seed mean 77.6 broad / 85.0 v5 still beats Qwen 74.1/81.7). HP matrix front-runner: LoRA+ at base lr=1e-5 (broad 82.8 / v5 88.5, 0 fails, clears both seed bands) — needs its own seed-confirm before promotion.** Prior "Champion: Qwen3-4B T4-v5split 100/74/82" now the SPEED/deploy champion, Gemma the ACCURACY champion. Gemma4 fine-tune required a full-model-load + torch2.8 + regex-target fix (finding #106). Modal workspaces: sandy-36852 (out of credits) + lolrazh ($30, card required for L40S/A100 — T4/L4/A10G free-tier only); flaky local network cancels attached modal runs + two overlapping `modal run` clients collide (APP_STATE_STOPPED) — one clean client, no head-pipe, retry loops mandatory.)
