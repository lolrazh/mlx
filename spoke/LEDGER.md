...
98. **Apple's on-device Foundation Model is a real Spoke deployment candidate — with a guardrail asterisk.** Zero-shot core23: 22% (ties Qwen3-4B, beats every other small model's zero-shot), 0.33s avg latency (fastest ever), zero download/RAM cost (OS-managed ~3B). Access: `FoundationModels.framework` on macOS 26+ via Swift (`spoke/bench/apple_fm_shim.swift` + `apple_fm_benchmark.py`); macOS 27 adds an `fm` CLI + Python SDK + third-party model providers. Fine-tune path exists: Apple's adapter training toolkit (PyTorch, rank-32 LoRA, exports ~160MB `.fmadapter`) — needs ≥32GB training RAM (Modal works), and adapters are PINNED to base-model/OS versions (toolkit v26 ≠ OS 27). Risks: (a) guardrails refused 3/23 innocuous cleanup requests even in `permissiveContentTransformations` mode — letter-by-letter spell-outs pattern-match slur-spelling; unclear if adapters bypass this; (b) "Preserve profanity" requirement may be structurally incompatible with Apple's safety layer; (c) ~3B capacity suggests a Llama-3B-like ceiling (~91%) if fine-tuned.
