...
| Medium | qwen-moe-profile | Router-profile a Qwen3-series MoE (Qwen3-30B-A3B class) with `ifp/profile_experts.py` | User requested (2026-07-06) as ifp-lite comparison: does Spoke-traffic expert concentration replicate across MoE families or is it Gemma-4-specific? Same class-level router tap works on mlx-lm's qwen3_moe. Run after Gemma 4 baseline. | ifp-lite step 1 |
