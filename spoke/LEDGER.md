...
| **HIGH** | emoji-ablation-confirm | Re-run emoji ablation with 2-3 seeds (and/or larger test set) to confirm finding #105 (+6 pts non-emoji, single-seed 3-0) | If it holds: ship emoji-stripped model + separate emoji handling. If it was noise: emoji is neutral, keep it. Cheap (~$2, 4-6 short runs). Decides whether v6-data should DROP emoji rather than strengthen it. | — |
...
105. **Emoji-ablation (user hypothesis, 2026-07-06): stripping emoji from training IMPROVES non-emoji accuracy — directionally clean but single-seed.** Two Qwen3-4B champion-recipe runs, identical except 106 emoji rows (10%) removed from train; both validated on the SAME emoji-free valid set (only variable = the emoji rows). Benchmarked on broad58 (v2). **Non-emoji categories (54 ex): full=70% (38/54) vs noemoji=76% (41/54), +6 pts — and noemoji trained on 106 FEWER examples, so this is the STRONG form (smaller set still wins).** Paired discordance 3-0 (three flipped to correct: caps/multi/passthrough; ZERO regressions). Mechanism confirmed by a striking sub-result: **the full model (trained on 106 emoji rows) scored 2/4 on emoji test — IDENTICAL to the model that never saw emoji.** So the emoji data bought nothing even for emoji (base Qwen handles easy ones zero-shot; neither gets the hard ones) — pure LoRA-capacity tax, consistent with finding #102 (emoji = isolated circuit). CAVEAT: +3/54, single seed each; run-to-run noise is ±2-3 ex, McNemar p≈0.25 — SUGGESTIVE not conclusive. Perfect 3-0 direction + plausible mechanism make it worth acting on, but confirm with 2-3 seeds or larger test set first. **Product implication: ship the emoji-stripped model (better on the ~90% text-edit majority) + handle emoji separately (dedicated pass / rules / base-model zero-shot).** Data variants: `spoke/data/v5_ablation/`. Runs: spoke-abl-full, spoke-abl-noemoji.
