...
| Medium | ifp-lite | Expert-locality profiling on Gemma 4 26B A4B (MLX, 4-bit ~14.4 GB, fits 24 GB M4 batch-1) | IFP (arXiv 2501.02086) works when routing is prompt-stable; Spoke is one fixed task + narrow input distribution — best case. Step 1 (~day): log router decisions over v5 inputs, plot expert-usage histogram; mmap means resident memory itself measures locality. Step 2 (~week): prune cold experts → domain-shrunk model, validate broad58. Step 3 (research): trained mask predictor à la Apple — nobody has open-sourced this; high-value OSS side project. | — |
