...
| **Qwen3-T2-cloud** | 03-04 | Qwen3-4B-Instruct-2507 (Unsloth, Modal L40S) | LoRA | r=8 | adam | v4 (1201) | 2000 | — | **Cloud Unsloth + Qwen3-4B text-only. Packing enabled (1201→327 seqs, ~49 epochs vs local's ~6.6). Response-only masking. lora_dropout=0.0. MLX conversion needed rope_theta config fix. 35% bf16, 1.65s latency. Massive regression vs local T2-v4 (100%). Likely overtrained from packing (49 epochs) + no regularization (dropout=0).** |
...
| **Qwen3-T2-cloud** | **iter 2000** | **bf16** | **v2** | **23** | **35%** | **5** | **3** | **11** | **4** | **1.65s** | **Cloud Unsloth (Modal L40S). Same model/data as T2-v4 but packing+response masking+no dropout. 100% → 35% regression. Overtrained ~49 epochs vs local's ~6.6. See finding #50.** |
...
50. **Cloud Unsloth training ≠ local mlx-lm training.** Same model (Qwen3-4B), same data (v4 1201), same hyperparameters (r=8, adam, lr=1e-5, 2000 steps) — but local mlx-lm = 100%, cloud Unsloth = 35%. Three key differences: (1) Packing collapsed 1201 → 327 sequences, so 2000 steps = ~49 epochs vs local's ~6.6 — massive overtraining. (2) Response-only masking (Unsloth trains only on assistant tokens, mlx-lm trains on full sequence). (3) lora_dropout=0.0 (Unsloth fast-path requirement) vs 0.05 locally. To use cloud effectively, must compensate: reduce steps proportionally to packing ratio (2000/7 ≈ 285 steps), or disable packing. Also: Unsloth's `save_pretrained_merged` mangles config.json (nests rope_theta, strips bos_token_id, changes eos_token_id format) — must patch before `mlx_lm.convert`.
