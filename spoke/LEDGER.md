...
| **HIGH** | afm-adapter | Train one Apple FM adapter on v5-split (toolkit 26.0.0, Modal Linux GPU) | Decides Apple-rail viability in one run: does 21% broad58 reach 3B-class ~85%, and do guardrail refusals (10/58 zero-shot) survive fine-tuning? Toolkit ships base weights; JSONL chat format ≈ our data. **Mechanics confirmed 2026-07-06 (finding #103): install = user gets ONLY the ~160MB `.fmadapter` via Background Assets (base model is OS-resident — exactly like the speech models); runtime API = `SystemLanguageModel.Adapter(name:)` → `SystemLanguageModel(adapter:)` → `LanguageModelSession`. Train = LoRA PEFT, JSONL `[{"role":"user"...},{"role":"assistant"...}]` (≈ our v5, one rename), Linux GPU → Modal works, Apple example recipe `epochs 5 / lr 1e-3 / batch 4` (lr 100x ours — respect their default first), optional draft model for spec-decode, `export_fmadapter` → package. Prod deploy needs `com.apple.developer.foundation-model-adapter` entitlement (NOT needed to train/test).** Only blocker: user downloads toolkit v26.0.0 (Apple Developer login-gated). See findings #98, #103. | User downloads toolkit |
