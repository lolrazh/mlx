...
> Last updated: 2026-03-03 (8-bit QLoRA = 96% but slower than bf16. Muon already in mlx-lm — config change only)
...
| **HIGH** | **Muon** | **Train T3-v4 with Muon optimizer** | **Already in mlx-lm — just `optimizer: muon` in config.yaml. 2x compute efficiency over Adam. Muon-trained models lose only ~0.5% on quantization vs >3% for Adam (arxiv 2601.09865). Needs higher LR (~0.02 range vs 1e-5 for Adam). Only applies to 2D params (LoRA matrices are 2D, so it's safe).** | Config change only |
...
45. **Muon optimizer: promising for quant robustness.** Research shows Muon-trained models lose ~0.5% accuracy on quantization vs >3% for Adam (arxiv 2601.09865). Riemannion (arxiv 2507.12142) is a LoRA-specific variant that treats adapters on a fixed-rank manifold — "consistent improvements in convergence speed and final task performance over standard LoRA." **Already in mlx 0.30.6 (`mlx.optimizers.Muon`) and wired up in mlx-lm 0.30.5 — just set `optimizer: muon` in config.yaml.** Needs higher LR (~0.02 range vs 1e-5 for Adam). Only applies to 2D weight matrices; embeddings/output still use Adam (LoRA matrices are all 2D, so safe for LoRA training). 9% → 87% (+78 pts, vs Llama's +61, Qwen3's +56). OOM'd without grad_checkpoint (18.9 GB) but with it only 11.6 GB — lowest training memory of any 4B model. Lowest val loss ever (0.056) but val loss still doesn't predict accuracy (iter 1000 at 0.094 scored 87% vs iter 500 at 0.056 scored 83%). Same self-correction #3 failure as Qwen3/LFM2 — only Llama gets this right. T11 (300 iters, 16 layers) → T11-ext (2000 iters, all 36 layers) = 83% → 91% (+8 pts). Two changes mattered: (1) all 36 layers — at iter 400, T11-ext already matched T11's 83% with same iter count; (2) more iters — accuracy kept climbing from 83% to 91% between iter 400 and 2000, long after val loss plateaued and started rising. Train loss hit literal 0.000 from iter 1200 but accuracy still improved. Fifth confirmation val loss is unreliable (finding #19). Only 2 remaining failures are both emphasis (CAPS vs **bold**) — a data format ambiguity, not a capability gap.
