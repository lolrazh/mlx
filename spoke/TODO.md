...
- **Persistent system-prompt cache** *(decided, not yet wired — no serving shell exists to wire into; see Track A)*: cache the fixed v2 system prompt once, feed only the per-request user suffix, trim back. Ref impl: `spoke/bench/pld_cache_bench.py`. Finding #113/#114. **MEASURED (clean, idle machine, g64 + lossless greedy): median 488 ms / mean 628 ms / p90 1207 ms, vs no-cache median ~928 ms — caching ~halves the median.** The p90 tail (long outputs) is the only thing over the 800 ms ceiling; tighten `max_new_tokens` or add a repetition guard to trim it.
